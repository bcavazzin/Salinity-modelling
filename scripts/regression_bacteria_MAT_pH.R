...
dat.2 <- dat.2[, c("bacteria", "log_salinity", "log_intensity", "MAT", 
                   "log_WpH", "Soi_pH", "IR6", "MBT5ME", "SpringT", "SummerT")] #for section 1 and 2
...
p.MBT <- ggplot(dat.2, aes(x = MAT, y = MBT5ME)) +
  geom_point() +
  geom_smooth(method = lm) +
  labs(x = "MAT", y = "MBT'5Me") +
  stat_cor(aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")),
           p.accuracy = 0.001, r.accuracy = 0.01) + 
  theme_bw()
p.MBT
...
p.MBT.spring <- ggplot(dat.2, aes(x = SpringT, y = MBT5ME)) +
  geom_point() +
  geom_smooth(method = lm) +
  labs(x = "Spring air temperature", y = "MBT'5Me") +
  stat_cor(aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")),
           p.accuracy = 0.001, r.accuracy = 0.01) + 
  theme_bw()
p.MBT.spring

p.MBT.summer <- ggplot(dat.2, aes(x = SummerT, y = MBT5ME)) +
  geom_point() +
  geom_smooth(method = lm) +
  labs(x = "Summer air temperature", y = "MBT'5Me") +
  stat_cor(aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")),
           p.accuracy = 0.001, r.accuracy = 0.01) + 
  theme_bw()
p.MBT.summer
